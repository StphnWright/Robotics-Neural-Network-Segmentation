...
def train(model, device, train_loader, criterion, optimizer):
    ...
    for _, data in enumerate(train_loader, 0):
        ...
        # Compute mIoU
        mIoU_all = iou(output, data['target'])
        mIoU = np.sum(mIoU_all)/len(mIoU_all)
    ...
    train_iou = np.mean(np.array(mIoU_history))
...
def val(model, device, val_loader, criterion):
    ...
    val_iou = np.mean(np.array(mIoU_history))
...
def main():
    ...
    # Prepare Dataloaders. Only shuffle the training set. You can use check_dataloader(your_dataloader) to check your implementation.
    ...
    # Remember to include the saved learning curve plot in your report
    train_loss_list, train_miou_list, val_loss_list, val_miou_list = list(), list(), list(), list()
    epoch, max_epochs = 1, 50  # You may want to make changes here
    best_miou = float('-inf')
