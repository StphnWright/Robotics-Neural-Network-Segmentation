...
def gen_obj_depth(obj_id, depth, mask):
    ...
    if (obj_id == -1):
        # All objects: keep everything except the background
        obj_depth = np.where(mask != 0, depth, np.zeros_like(depth))
    else:
        # Specific object: keep only the object
        obj_depth = np.where(mask == obj_id, depth, np.zeros_like(depth))
...
def obj_depth2pts(obj_id, depth, mask, camera, view_matrix):
    ...
    # Get the depth for just the requested object
    obj_depth = gen_obj_depth(obj_id, depth, mask)
    
    # Convert to point cloud
    pts_camera = depth_to_point_cloud(camera.intrinsic_matrix, obj_depth)
    
    # Get the conversation matrix for camera to world
    wTc = cam_view2pose(view_matrix)
    
    # Transform points from camera to world coordinates
    world_pts = transform_point3s(wTc, pts_camera)
...
def align_pts(pts_a, pts_b, max_iterations=20, threshold=1e-05):
    ...
    # Attempt Procrustes, return if there is an error
    try:
        initial_matrix = trimesh.registration.procrustes(pts_a, pts_b, reflection = False, scale = False)[0]
    except np.linalg.LinAlgError:
        return None
    #initial_matrix = np.eye(4)
    #initial_matrix = np.rand(4, 4)
    
    # Trimesh registration
    matrix = trimesh.registration.icp(pts_a, pts_b, initial = initial_matrix, threshold = threshold, 
                                      max_iterations = max_iterations, reflection = False, scale = False)[0]
...
def estimate_pose(depth, mask, camera, view_matrix):
    ...
    # Loop over each object (1 to 5)
    for i in range(1, 6):
        
        # Get projected and sample points
        pts_projected = obj_depth2pts(i, depth, mask, camera, view_matrix)
        pts_sample = obj_mesh2pts(i, pts_projected.shape[0])
        
        # Align the points
        obj_pose = align_pts(pts_sample, pts_projected, max_iterations = 40, threshold = 1e-7)
        
        # Record the pose in the list
...
def main():
    ...
    for scene_id in range(5):
        ...
        # Process the ground truth mask for validation only
        if args.val:
            # Get the object poses
            mask_gt = image.read_mask(dataset_dir + 'gt/' + str(scene_id) + '_gt.png')
            list_obj_pose_gt = estimate_pose(depth, mask_gt, my_camera, view_matrix)
            
            # Export plies
            export_gt_ply(scene_id, depth, mask_gt, my_camera, view_matrix)
        ...
        # Process the predicted mask for both validation and test
        if args.val or args.test:
            # Get the object poses
            mask_pred = image.read_mask(dataset_dir + 'pred/' + str(scene_id) + '_pred.png')
            list_obj_pose_pred = estimate_pose(depth, mask_pred, my_camera, view_matrix)
